...
class MyProductos implements ActiveRecord {
    ...
    public function buscarTodo() {
        $sql = 'SELECT p.id, p.cod_barra, p.descripcion, p.umed_id, p.paquete, '
                . 'p.punto_reposicion, p.habilitado, p.categoria_id FROM productos p;';
        $resultado = mysqli_query($_SESSION['con'], $sql);
        $fila = mysqli_fetch_object($resultado);
        if ($fila) {
            $aProducts = array();
